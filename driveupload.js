      ...
      drive.files.list({
        ...
        auth: this.jwtClient
          ...
          .then((folderList) => {
            ...
            this.AddFileToDrive(file,address,name,foundFolder)
                .then((results) => {
                  resolve(results.webViewLink);
                })
                .catch((err) => {
                  reject(Error("From Upload after list: "+err));
                })
          })
          .catch((err) => {
            reject(Error("From Upload before list: "+err));
          });
    });
  }
  AddFileToDrive(file, address, fileName, folderID) {
    return new Promise((resolve, reject) => {
